...
import { deleteIcon, starIcon, threeDotsVerticalIcon } from '../../assets'
...
const AddCustomThemeModal = (props: AddCustomThemeModalProps) => {
    ...
    return (
        <>
            <div id='modal-bg' onClick={(e) => handleCloseModal(e)} className='w-screen z-[0] h-screen fixed top-0 right-0 bg-black/50'></div>
            <div className="flex flex-row-reverse gap-8 p-6 fixed z-1 inset-0 h-max w-fit m-auto rounded">
                <div style={{ backgroundColor: theme.primaryColour, color: theme.primaryTextColour }} id='add-todo-form-modal' className='p-6 w-[460px] h-max m-auto rounded'>
                    <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
                        <div className="flex flex-col flex-1 gap-2">
                            <label htmlFor="themeName" className='font-medium'>Theme name</label>
                            <input onChange={(e) => setNewTheme(state => ({ ...state, themeName: e.target.value.toLowerCase() }))} style={{ backgroundColor: theme.secondaryColour }} placeholder='Theme name' type="text" id='themeName' className='rounded w-full px-3 py-2' />
                        </div>
                        ...
                        <div className="submit-btn-wrapper w-full mt-2">
                            <button type="submit" className='w-full px-4 py-3 text-white bg-[#4F46E5] rounded'>Save Theme Preset</button>
                        </div>
                    </form>
                </div>
                        ...
                        <div style={{ backgroundColor: newTheme.primaryColour }} className='todo-item flex flex-col w-full max-w-[336px] min-h-[260px] p-[25px] rounded-md bg-[#FFFFFF]'>
                            ...
                            <div className="mt-4 flex items-center">
                                <input
                                    ...
                                    className='hidden'
                                    type="checkbox" />
                                <label className='select-none flex flex-1 gap-2 text-base leading-[20px] min-w-[160px] font-normal text-[#18AB1E]' htmlFor='isCompleted'><div className='h-6 w-6 rounded-lg bg-[#18AB1E]'></div>completed</label>
                                <div className="flex gap-2">
                                    <input id='isImportant' className='hidden' type="checkbox" checked={true} />
                                    <label style={{ color: newTheme.iconColour }} className={`hasHoverEffect p-1 rounded`} htmlFor='isImportant'>{starIcon}</label>
